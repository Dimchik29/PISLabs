# Laboratory Works on "Applied Intelligent Systems"

## Project: Development of Software for Autonomous Water Body Scanning Using the Apache4 Robotic Boat

**Educational Institution:**  
Brest State Technical University  
Department of Intelligent Information Technologies  

**Students:**  
- Petrushchik D. A.  
- Turich D. A.  

**Group:** II-26  

**Supervisor:** Savchuk Yu. N.  

---

## Overview

This repository contains the laboratory work developed as part of the course "Applied Intelligent Systems." The project focuses on creating software for autonomous water body scanning using the Apache4 robotic boat.

---

## Laboratory Works

| # | Title | Link |
|---|-------|------|
| 1 | Laboratory Work No. 1 – Project Plan | [View](https://github.com/Jewgien/Dev-of-the-robot-Keenon/blob/main/Laboratory%20work%20%E2%84%961%20PIS.docx) |
| 2 | Laboratory Work No. 2 – Hardware Analysis | [View](https://github.com/Jewgien/Dev-of-the-robot-Keenon/blob/main/Laboratory%20work%20%E2%84%962%20PIS.docx) |
| 3 | Laboratory Work No. 3 – Navigation Design | [View](https://github.com/Jewgien/Dev-of-the-robot-Keenon/blob/main/Laboratory%20work%20%E2%84%963%20PIS.docx) |
| 4 | Laboratory Work No. 4 – Implementation | [View](https://github.com/Jewgien/Dev-of-the-robot-Keenon/blob/main/Laboratory%20work%20%E2%84%964%20PIS.docx) |

---

## Contents

### [Laboratory Work No. 1 – Project Plan](https://github.com/Jewgien/Dev-of-the-robot-Keenon/blob/main/Laboratory%20work%20%E2%84%961%20PIS.docx)

**Topic:** Project Plan: Software Development for Autonomous Water Body Scanning Using the Apache4 Robotic Boat

**Objective:** Develop a phased plan for creating software...

**Contents:**
- Stage 1 – Analysis (Lab No. 2)
- Stage 2 – Design (Lab No. 3)  
- Stage 3 – Implementation (Lab No. 4)

---

### [Laboratory Work No. 2 – Hardware Analysis](https://github.com/Jewgien/Dev-of-the-robot-Keenon/blob/main/Laboratory%20work%20%E2%84%962%20PIS.docx)

**Topic:** Development of Software for Autonomous Water Body Scanning Using the Apache4 Robotic Boat

**Objective:** Study the hardware structure, main components, and interfaces of the Apache4 robotic boat...

**Key Components Studied:**
- Hull and Propulsion System
- Sensors (SBES, ADCP, 360° camera)
- Navigation (GPS/GLONASS, IMU)
- Communication interfaces
- Charging methods

---

### [Laboratory Work No. 3 – Navigation Design](https://github.com/Jewgien/Dev-of-the-robot-Keenon/blob/main/Laboratory%20work%20%E2%84%963%20PIS.docx)

**Topic:** Design of Autonomous Navigation Parameters and Water Body Scanning Route Maps

**Objective:** Learn to configure the Apache4 robotic boat, modify motion parameters, and create route maps...

**Key Tasks:**
- Interface and telemetry settings
- Sound and notification configuration
- Creating route maps with Mapping Tool
- Waypoint placement (Base, Scan, Return, Obstacle)

---

### [Laboratory Work No. 4 – Implementation](https://github.com/Jewgien/Dev-of-the-robot-Keenon/blob/main/Laboratory%20work%20%E2%84%964%20PIS.docx)

**Topic:** Implementation of Practical Autonomous Operation Modes of the Apache4 Robotic Boat

**Objective:** Master the main autonomous operation modes...

**Operating Modes:**
- Survey Mode
- Area Scan Mode
- Obstacle Avoidance Mode
- Return to Base Mode
- Monitoring Mode

---

## Repository Structure
